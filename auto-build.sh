...
echo yes | android update sdk -a -u -t build-tools-20.0.0 > /dev/null || exit 1

echo "Installing android..."
echo yes | android update sdk --filter android-20 --no-ui --force > /dev/null || exit 1
echo yes | android update sdk --filter addon-google_apis_x86-google-20 --no-ui --force > /dev/null || exit 1
...
mv cachedmodel/build/reports /tmp/build/cachedmodel/

mkdir -p /tmp/build/example-api
mv example-api/build/libs /tmp/build/example-api/
mv example-api/build/reports /tmp/build/example-api/

mkdir -p /tmp/build/example-app
mv example-app/build/outputs /tmp/build/example-app/
